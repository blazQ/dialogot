...
func _ready():
	...
	giovanni.voice = preload("res://addons/dialogot/VoiceProfiles/high_fast_voice.tres")

	var lucifero := CharacterProfile.new()
	lucifero.display_name = "Lucifero"
	lucifero.voice = preload("res://addons/dialogot/VoiceProfiles/deep_slow_voice.tres")

	var narratore := CharacterProfile.new()
	narratore.display_name = "Narratore"
	narratore.voice = preload("res://addons/dialogot/VoiceProfiles/standard_voice.tres")
	...
	var file := FileAccess.open("res://examples/Dialogues/Example.dialogue", FileAccess.READ)
